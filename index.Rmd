...
En el siguiente análisis a presentar, se trata de analizar las razones que pueden haber influenciado la migración global para el año 2022. Esto permitiría entender qué posibles factores dentro de los países son afectan de forma negativa la percepción de un país en particular y ello resulta en una migración de sus ciudadanos. Para lograr este objetivo, se han realizado regresiones lineales para determinar cómo diferentes modelos con diferentes factores funcionan. Asimismo, se presentan técnicas para explicar las dimensiones o variables a utilizar y poder determinar cuáles de ellas resultan parecidas entre sí y afecten similarmente a la variable latente. Finalmente, se presenta un análisis de conglomerados para visualizar la similitud entre casos, o países presentados, y que puedan poseer características similares. Ello permite entender qué países son los que pueden poseer políticas o niveles de calidad de vida parecidas y en la forma en la que puedan afectar a la población.
...
psych::KMO(corMatrix) 
```
Vemos que el Overall MSA es de 0.78. Al ser un valor superior a 0.6, podemos afirmar que aporta al análisis. 
...
Como parte del proceso, elaboramos las pruebas de matriz de identidad y matriz de singularidad: 
...
```
Como ambas resultan falsas, podemos proceder a aplicar el código para la sugerencia del número de factores. 

Es relevante señalar que, pese a que la normalización de las variables no es exigida para la elaboración del EFA, en el presente trabajo, decidimos optar por ella. Esto obedece a un motivo específico: Sin realizarla, el número de factores que se nos sugiere para redimensionar la base de datos es 1. Dicha alternativa no aporta a nuestra investigación. Con las variables normalizadas, observamos que se nos sugiere dividir las variables en dos factores. 
...
```
Vemos que, en el proceso de redimensionar 8 variables a 2, se ha logrado recuperar el 43% de información.

Sobre las variables independientes, tenemos lo siguiente: Tanto "Criminalidad" como "P.Corrupción_2021", "E2..Economic.Inequality", "EmisionesCO2", "Paz" y "Catastrofe" aparecen en ambos factores. Sin embargo, estableceremos 0.5 como un punto de corte para determinar a qué factor corresponderá cada componente. En los casos de "Desempleo_2021" y de "PBI_2020", encontramos que ambos indicadores solamente pertenecen a un factor: el segundo y el primero, respectivamente. 
...
En cuanto a las demás variables, todas han sido ubicadas en un solo factor con excepción de "Paz", pues los dos valores que presenta se encuentran por encima del punto de corte. 

```{r echo=FALSE}
fa.diagram(resfa,main = "Resultados del EFA")
```

A continuación, realizamos las pruebas de comunalidad y de complejidad. 
```{r echo=FALSE}
...
```
En cuanto a la complejidad de las variables, es relevante señalar que "Paz", así como contribuye a los factores, también presenta tal complejidad que se acerca a más de uno. Otros componentes que también demuestran complejidad son "Catastrofe" y, en menor medida, "Criminalidad". 
